                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($checkout_detail as $cd)
                                        <tr>
                                            <td>{{ $no++ }}</td>
                                            <td><a href="/products/{{ $cd->products->slug }}" class="nav-link"
                                                    style="color:black">{{ $cd->products->nm_products }}</a></td>
                                            <td style="text-align: center">
                                                <form action="{{ route('update.cart') }}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{ $cd->id }}">
                                                    <input type="number" value="{{ number_format($cd->quantity) }}"
                                                        class="form-control quantity update-cart" name="quantity">
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="6" class="text-end">
                                            <h3><strong>Total Rp.{{ number_format($checkout->subtotal ?? 0) }}</strong>
                                            </h3>
                                        </td>
                                    </tr>
                                    <tr>
